...
setup_kernelsu() {
    echo "[+] Setting up SingkoSU..."
    test -d "$GKI_ROOT/KernelSU" || git clone https://github.com/skglab-dev/SingkoSU && echo "[+] Repository cloned."
    cd "$GKI_ROOT/SingkoSU"
    ...
    cd "$DRIVER_DIR"
    ln -sf "$(realpath --relative-to="$DRIVER_DIR" "$GKI_ROOT/SingkoSU/kernel")" "kernelsu" && echo "[+] Symlink created."
